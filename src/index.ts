...
const createBattlestation = (scene: Scene): Mesh => {
  ...
  const ring = MeshBuilder.CreateTorus(
    'ring',
    { diameter: STATION_SIZE * 1.5, thickness: STATION_SIZE / 10 },
    scene
  );
  ...
  const ring2 = MeshBuilder.CreateTorus(
    'ring2',
    { diameter: STATION_SIZE * 1.2, thickness: STATION_SIZE / 15 },
    scene
  );
  ...
  for (let i = 0; i < 5; i++) {
    const module = MeshBuilder.CreateBox(
      'module' + i,
      {
...
const createAttacker = (scene: Scene): IAttacker => {
  // Create main body
  const attackerMesh = MeshBuilder.CreateBox(
    'attacker',
    { width: ATTACKER_SIZE, height: ATTACKER_SIZE / 2, depth: ATTACKER_SIZE * 1.5 },
    scene
  );
  ...
  const leftWing = MeshBuilder.CreateBox(
    'leftWing',
    { width: ATTACKER_SIZE * 0.8, height: ATTACKER_SIZE / 10, depth: ATTACKER_SIZE * 0.6 },
    scene
  );
  ...
  // Add fins
  const topFin = MeshBuilder.CreateBox(
    'topFin',
    { width: ATTACKER_SIZE / 10, height: ATTACKER_SIZE * 0.6, depth: ATTACKER_SIZE * 0.5 },
    scene
  );
  topFin.parent = attackerMesh;
  topFin.position = new Vector3(0, ATTACKER_SIZE * 0.4, -ATTACKER_SIZE * 0.4);

  const bottomFin = MeshBuilder.CreateBox(
    'bottomFin',
    { width: ATTACKER_SIZE / 10, height: ATTACKER_SIZE * 0.6, depth: ATTACKER_SIZE * 0.5 },
    scene
  );
  ...
  const leftThruster = MeshBuilder.CreateCylinder(
    'leftThruster',
    { height: ATTACKER_SIZE * 0.4, diameter: ATTACKER_SIZE * 0.2 },
    scene
  );
  leftThruster.parent = attackerMesh;
  leftThruster.position = new Vector3(
    -ATTACKER_SIZE * 0.3,
    -ATTACKER_SIZE * 0.1,
    -ATTACKER_SIZE * 0.8
  );
  ...
  const rightThruster = MeshBuilder.CreateCylinder(
    'rightThruster',
    { height: ATTACKER_SIZE * 0.4, diameter: ATTACKER_SIZE * 0.2 },
    scene
  );
  rightThruster.parent = attackerMesh;
  rightThruster.position = new Vector3(
    ATTACKER_SIZE * 0.3,
    -ATTACKER_SIZE * 0.1,
    -ATTACKER_SIZE * 0.8
  );
...
const updateAttacker = (
  ...
  const canEvade =
    !attacker.isEvading ||
    currentTime - attacker.lastEvasionTime > attacker.transitionDuration * 0.8;
  ...
  if (canEvade) {
    for (const missile of missiles) {
      ...
      if (
        distance < attacker.evasionThreshold &&
  ...
  if (needsEvasion && closestMissile) {
    ...
    const perpVector = new Vector3(evadeDirection.y, -evadeDirection.x, evadeDirection.z)
      .normalize()
      .scale(evasionDistance * perpComponent);
    ...
    if (Math.random() < 0.3) {
      ...
      attacker.evasionThreshold = getRandomFloat(
        EVASION_THRESHOLD_MIN + 3,
        EVASION_THRESHOLD_MAX - 3
      );
    }
  ...
  // Check if we're close to the target or if we've been moving toward the same target for too long
    ...
    Vector3.Distance(attacker.mesh.position, attacker.target) < 25 ||
  ...
  // Extremely low probability to minimize random shaking - only for long-term variation
    ...
    Math.random() < 0.0001 &&
    currentTime - attacker.targetChangeTime > attacker.transitionDuration * 5
  ) {
    // Save current target before updating
    attacker.currentTarget = attacker.target.clone();
    attacker.targetChangeTime = currentTime;
...
const createRotationQuaternion = (
  direction: Vector3,
